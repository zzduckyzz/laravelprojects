        ...
        <div class="box">
            ...
            <div class="box-body">
                <div class="col-md-12 mg-t-10">
                    <form class="form-horizontal" action="{{ route('editCategories', $categoies->id) }}" method="POST" enctype="multipart/form-data">
                        <div class="col-md-12">
                            <div class="form-group {{ $errors->first('name') ? 'has-error' : '' }} ">
                                <label for="inputEmail3" class="col-sm-3 control-label default">Tên danh mục<sup class="title-sup">(*)</sup></label>
                                <div class="col-sm-9">
                                    <input type="text" class="form-control"  placeholder="Tên danh mục" name="name" value="{{ old('name', $categoies->name) ? old('name', $categoies->name) : NULL }}">
                            ...
                            <div class="form-group {{ $errors->first('position') ? 'has-error' : '' }} ">
                                ...
                                <div class="col-sm-9">
                                    <input type="number" class="form-control"  placeholder="Vị trí hiển thị (1, 2, 3, ...)" name="position" min="0" value="{{ old('position', $categoies->position) ? old('position', $categoies->position) : NULL }}">
                            ...
                            <div class="form-group {{ $errors->first('status') ? 'has-error' : '' }} ">
                                ...
                                <div class="col-sm-9">
                                    <select name="status" id="status"   class="form-control" style="width: 100%;" tabindex="-1">
                                        ...
                                        {{ old('status', $categoies->status) ==  STATUS_ACTIVE ? 'selected ="selected"' : '' }}
                                        >{{trans('message_vn.show')}}</option>
                                        <option value="{{STATUS_LOCKED}}"
                                        {{ old('status', $categoies->status) ==  STATUS_LOCKED ? 'selected ="selected"' : '' }}
